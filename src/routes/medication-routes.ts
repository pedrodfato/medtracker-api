...
export async function medicationsRoutes(app: FastifyInstance) {
    ...
    app.get('/medications', {preHandler: [verifySession]}, async (request, reply) => {
        try {
            const myMedications = await db.select({id: medications.id, name: medications.name, dosage: medications.dosage, totalPills: medications.totalPills, frequencyHours: medications.frequencyHours, startDate: medications.startDate}).from(medications);
